 ...
 */
public class PickupShelfFar extends SequentialCommandGroup {
 
  public PickupShelfFar(ElevatorSubsystem elevator, ElbowSubsystem elbow,  ClawArmsSubsystem clawArms ) {
    addCommands(
      Commands.sequence(
        new OpenClawWithWait(clawArms),                                         // pneumatic claw needs time to open

        Commands.parallel(
          new ElevatorPIDCmd(elevator, ElevatorConstants.kShelfFarHeight),         // move elev to shelf pos - note that its faster than the elbow
          new ElbowPIDCmd(elbow, ElbowConstants.kRetreiveFromShelfFarAngle,  ElbowConstants.kMaxVel, ElbowConstants.kMaxAcc, ElbowConstants.kTolerance)  ),    // move elbow to shelf pos - note that its slower than elev
...
//        new WaitCommand(0.1),          //TODO: Calibrate time           // settle down arm - it rocks when stopped
        ...
        new ElevatorPIDCmd(elevator, (ElevatorConstants.kShelfFarHeight)+5.0),     // raise elev to ensure cone/cube does not hit shelf when returning arm
