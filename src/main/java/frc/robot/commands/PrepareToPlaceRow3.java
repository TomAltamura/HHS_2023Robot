...
public class PrepareToPlaceRow3 extends SequentialCommandGroup {
 
  public PrepareToPlaceRow3(ElevatorSubsystem elevator, ElbowSubsystem elbow, ClawArmsSubsystem clawArms ) {
    addCommands(
...
//        ),
        ...
        //    do a multistep move to not go above the required height rule, next step move to Node 3 position
        Commands.parallel(
          new ElbowPIDCmd(elbow, ElbowConstants.kPlaceOnRow3Angle,  ElbowConstants.kMaxVel, ElbowConstants.kMaxAcc, 0),  // No Tolerance - keep PID active to try to keep elbowangle from moving     

          Commands.sequence(
            new WaitCommand(0.25),    // Wait for elbow to move tostay under max height
